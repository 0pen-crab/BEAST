...
Check if `RELEASE.md` exists in the project root. If it does, read it — this is a running draft maintained via the `/add-notes` skill with pre-written user-facing descriptions. Use these entries as-is (they're already in the correct style). Merge with other sources to ensure nothing is missed.
