...
export async function loadMetadataJsonl(metadataPath: string, cancelSignal?: AbortSignal): Promise<FileMetadata[]> {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      malformed++;
      if (!sample) sample = trimmed.slice(0, 200);
    }
  }
  if (malformed > 0) {
    // The metadata file is generated by our own mirror-builder — malformed
    // lines mean a builder bug, and silently skipping them would silently
    // shrink scan coverage. Fail loudly instead.
    throw new Error(
      `Mirror metadata at ${metadataPath} contains ${malformed} malformed JSONL line(s) — first: ${sample}`,
    );
  }
  return files;
}
