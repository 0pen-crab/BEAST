    ...
    for (const row of rows) {
      try {
        ...
        try {
          ...
          // getSource/updateSource themselves failed — DB trouble; the poll
          // loop retries next cycle and /api/health surfaces a down DB.
          console.error(`[sync] Failed to record sync failure for source ${row.id}:`, eventErr instanceof Error ? eventErr.message : eventErr);
        }
      }
    }
  } catch (err) {
    // DB unreachable — nothing else can be written; poll retries next cycle
    // and /api/health surfaces the DB outage.
